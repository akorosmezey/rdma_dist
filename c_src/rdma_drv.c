...
static bool rdma_drv_init_ibverbs(RdmaDrvData* data)
{
	...
	data->send_mr =
		ibv_reg_mr(
			data->pd, data->send_buffers.buffers, data->options.buffer_size * (data->options.num_buffers - 1),
	...
	data->recv_mr = ibv_reg_mr(
		data->pd, data->recv_buffers.buffers,
		data->options.buffer_size * data->options.num_buffers, IBV_ACCESS_LOCAL_WRITE);
	if (!data->recv_mr)
	{
	...
	else
	{
		...
		if (remaining <= data->options.buffer_size)
		{
			...
			if (data->options.active)
			{
				...
				{
					driver_output2(
						data->port, output_buffer, data->options.packet,
						output_buffer + data->options.packet, output_buffer_size - data->options.packet);
				}
				else
				{
			...
			else
			{
				ErlDrvTermData spec[] = {
					...
					data->options.binary ? ERL_DRV_BUF2BINARY : ERL_DRV_STRING, (ErlDrvTermData) output_buffer,
					output_buffer_size,
					ERL_DRV_TUPLE, 2,
					ERL_DRV_TUPLE, 2,
				};
...
static ErlDrvSSizeT rdma_drv_control(
	ErlDrvData drv_data, unsigned int command, char* buf, ErlDrvSizeT len, char** rbuf,
	ErlDrvSizeT rlen)
{
	RdmaDrvData* data = (RdmaDrvData*) drv_data;
